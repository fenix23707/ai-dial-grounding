...
async def main():
    ...
    if user_question:
        ...
        batch_size = 100
        all_users_batches = (all_users[i:i + batch_size] for i in range(0, len(all_users), batch_size))

        tasks = []
        for batch_users in all_users_batches:
            user_message = USER_PROMPT.format(context=join_context(batch_users), query=user_question)
            tasks.append(generate_response(BATCH_SYSTEM_PROMPT, user_message))

        responses = await asyncio.gather(*tasks)
        filtered_responses = [response for response in responses if response != 'NO_MATCHES_FOUND']
        if filtered_responses:
            ...
            ## USER QUESTION: 
            {user_question}
            """.format(user_question=user_question, retrieved_result='\n\n'.join(filtered_responses))

            print(await generate_response(FINAL_SYSTEM_PROMPT, message))
        else:
